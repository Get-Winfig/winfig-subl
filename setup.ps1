...
function Test-WindowsSublimeTextInstalled {
    # Try common installation paths first
    $possiblePaths = @(
        "C:\Program Files\Sublime Text",
        "C:\Program Files\Sublime Text 3",
        "C:\Program Files\Sublime Text 4",
        "${env:LocalAppData}\Sublime Text"
    )

    foreach ($path in $possiblePaths) {
        $sublPath = Join-Path $path "subl.exe"
        if (Test-Path $sublPath) {
            Log-Message -Message "Sublime Text found at: $sublPath" -Level "SUCCESS"
            return $true
        }
    }
    ...
    try {
        subl --version *> $null
        Log-Message -Message "Sublime Text is installed and in PATH." -Level "SUCCESS"
        return $true
    } catch {
        Show-ErrorMessage "Sublime Text is not installed or not found in PATH."
        Log-Message -Message "Sublime Text is not installed or not found in PATH." -Level "ERROR"
        return $false
    }
}
...
Test-WindowsSublimeTextInstalled | Out-Null
Show-SuccessMessage "Windows Sublime Text installation check completed."
...
Winfig-Banner
Write-SectionHeader -Title "Cloning Winfig Sublime Text Repository"
Write-Host ""
$repoPath = Join-Path $Global:WinfigPaths.DotFiles "winfig-subl"
if (-not (Test-Path -Path $repoPath)) {
    try {
        Show-InfoMessage "Cloning Winfig Sublime Text repository..."
        Log-Message -Message "Cloning Winfig Sublime Text repository..." -Level "INFO"
        git clone https://github.com/Get-Winfig/winfig-subl.git $repoPath *> $null
    } catch {
        Show-ErrorMessage "Failed to clone Winfig Sublime Text repository: $($_.Exception.Message)"
        Log-Message -Message "Failed to clone Winfig Sublime Text repository: $($_.Exception.Message)" -Level "ERROR"
        exit 1
    }
    if (Test-Path -Path $repoPath) {
        Show-SuccessMessage "Cloned Winfig Sublime Text repository to $repoPath."
        Log-Message -Message "Cloned Winfig Sublime Text repository to $repoPath." -Level "SUCCESS"
    } else {
        Show-ErrorMessage "Winfig Sublime Text repository was not cloned. Please check your internet connection or repository URL."
        Log-Message -Message "Winfig Sublime Text repository was not cloned. Please check your internet connection or repository URL." -Level "ERROR"
        exit 1
    }
} else {
    try {
        Show-InfoMessage "Updating Winfig Sublime Text repository..."
        Log-Message -Message "Updating Winfig Sublime Text repository..." -Level "INFO"
        Push-Location $repoPath
        git pull *> $null
        Pop-Location
        Show-SuccessMessage "Updated Winfig Sublime Text repository at $repoPath."
        Log-Message -Message "Updated Winfig Sublime Text repository at $repoPath." -Level "SUCCESS"
    } catch {
        Show-ErrorMessage "Failed to update Winfig Sublime Text repository: $($_.Exception.Message)"
        Log-Message -Message "Failed to update Winfig Sublime Text repository: $($_.Exception.Message)" -Level "ERROR"
        exit 1
    }
}
...
Winfig-Banner
Write-SectionHeader -Title "Symbolic Linking Sublime Text Configuration"
Write-Host ""
try {
    $sublimeTarget = $null
    $possiblePaths = @(
        "${env:AppData}\Sublime Text\Sublime Text 4",
        "${env:AppData}\Sublime Text\Sublime Text 3",
        "${env:AppData}\Sublime Text",
        "${env:LocalAppData}\Sublime Text\Sublime Text 4",
        "${env:LocalAppData}\Sublime Text\Sublime Text 3",
        "${env:LocalAppData}\Sublime Text"
    )
    ...
    foreach ($path in $possiblePaths) {
        if (Test-Path $path) {
            $sublimeTarget = $path
            Log-Message -Message "Found Sublime Text at: $sublimeTarget" -Level "INFO"
            break
        }
    }
    $ThemesDir          = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Themes")
    $emmetSettings      = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Emmet.sublime-settings")
    $packageControl     = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Package Control.sublime-settings")
    $preferences        = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "Preferences.sublime-settings")
    $statusBarFileSize  = [System.IO.Path]::Combine($Global:WinfigPaths.Templates, "StatusBarFileSize.sublime-settings")
    $target             = [System.IO.Path]::Combine($Global:WinfigPaths.AppDataRoaming, $sublimeTarget, "Packages\User")

    $files = @{
        "Emmet.sublime-settings"              = $emmetSettings
        "Package Control.sublime-settings"    = $packageControl
        "Preferences.sublime-settings"        = $preferences
        "StatusBarFileSize.sublime-settings"  = $statusBarFileSize
        "Themes"                              = $ThemesDir
    }
...
Write-Host ""
Write-SectionHeader -Title "Thank You For Using Winfig Sublime Text" -Description "https://github.com/Get-Winfig/"
